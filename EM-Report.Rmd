...
plot(x, graphC$yTotal, ylim = c(0,6))
```
...
```{r}
microbenchmark(EM(x,yComplete,thresh = 0.2),
               optim(par =  eval(parse(text = 'param <- getStart(x, yComplete)')),
                     x = x, yUncensored = yComplete, 
                    fn = logmax, method = 'BFGS', hessian = TRUE),
               optim(par = c(2,2,6), x = x, yUncensored = yComplete, 
                   fn = logmax, method = 'BFGS', hessian = TRUE))
```
